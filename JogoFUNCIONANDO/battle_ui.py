...
class UI:
    def __init__(self, monster, player_monsters, simple_surfs, get_input):
        ...
        # control 
        self.general_options = ['atacar', 'curar', 'trocar', 'fugir']  # Traduzido para português
    ...
    def input(self):
        ...
        if self.state == 'general':
            ...
            if keys[pygame.K_SPACE]:
                # Mapear opções em português para inglês
                option_map = {'atacar': 'attack', 'curar': 'heal', 'trocar': 'switch', 'fugir': 'escape'}
                selected_option = self.general_options[self.general_index['col'] + self.general_index['row'] * 2]
                self.state = option_map[selected_option]
            ...
            if self.available_monsters:
                ...
                if keys[pygame.K_SPACE]:
                    self.get_input('switch', self.available_monsters[self.switch_index])
                    self.state = 'general'
    ...
    def stats(self):
        ...
        name_rect = name_surf.get_rect(topleft = (rect.left + rect.width * 0.05, rect.top + 12))
